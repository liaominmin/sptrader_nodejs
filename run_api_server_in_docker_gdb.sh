...
ulimit -c unlimited && ulimit -a && . /node_env.sh && export PATH=/$NODE_VERSION/bin:$PATH && LD_LIBRARY_PATH=./ LD_PRELOAD=./libapiwrapper.so gdb -q -batch -ex "run" -ex "thread apply all backtrace" -ex "quit" -args node sptrader_api_server.js $* 2>&1
